...
# receives every non-command message in that chat. 
# Disable with exit_stream or the !exit command (mapped to exit_command).
...
def suppress_next(chat_id: str, text: str) -> None:
    """
    Mark the next outgoing message with this 
    text in this chat to be ignored by the dispatcher.
    """
...
def should_suppress(chat_id: str, text: Optional[str]) -> bool:
    """
    Return True and consume suppression if 
    this (chat, text) is marked for one-time ignore.
    """
