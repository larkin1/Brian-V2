...
def whitelist(data, client):
    ...
    for entry in raw_entries:
        ...
        # If necesary, format the number
        if (entry.isdigit() or
                (re.sub(r"\D", "", entry) == entry
                and not entry.endswith(".us"))):
    ...
    if added:
        client.sendText(
            data["chatId"], 
            f"Added to Whitelist: {", ".join(f"`{a}`" for a in added)}"
        )
        
    if already:
        client.sendText(
            data["chatId"], 
            f"Already in Whitelist: {", ".join(f"`{a}`" for a in already)}"
        )
...
def ban(data, client):
    ...
    if removed:
        client.sendText(
            data["chatId"], 
            f"Removed from Whitelist: {", ".join(f"`{a}`" for a in removed)}"
        )
        
    if not_found:
        client.sendText(
            data["chatId"], 
            f"Not found in Whitelist: {", ".join(f"`{a}`" for a in not_found)}"
        )